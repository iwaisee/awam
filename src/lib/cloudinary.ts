...
/** Upload a downscaled data URL to Cloudinary and return its secure CDN
    URL. Returns null when credentials are missing or the upload fails —
    callers fall back to storing the inline data URL, so a misconfigured
    cloud can never lose the evidence. */
export async function uploadReportImage(
  ...
  try {
    ...
    console.error(
      `[cloudinary] ${kind} upload failed, keeping inline data URL:`,
